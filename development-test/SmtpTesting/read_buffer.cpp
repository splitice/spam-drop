...
uint16_t rbuf_writen(struct read_buffer* buffer, const char* src, uint16_t n){
	uint16_t write_to_end = rbuf_write_to_end(buffer);
	uint16_t to_write;
	uint16_t written;

	if (n != 0){
		to_write = (n > write_to_end) ? write_to_end : n;

		memcpy(RBUF_WRITEPTR(buffer), src, to_write);

		n -= to_write;
		written = to_write;

		//Write circularly if we need to
		if (n != 0){
			write_to_end = rbuf_write_remaining(buffer);

			if (write_to_end != 0){
				to_write = (n > write_to_end) ? write_to_end : n;

				memcpy(buffer->buffer, src, to_write);
				written += to_write;
			}
		}
	}
	...
	//Move write pointer
	RBUF_WRITEMOVEPTR(buffer, written);
